...
const getType_1 = __webpack_require__(8);
const str = [
    '0',
    '1',
    '2',
...
exports.default = (res) => {
    try {
        ...
        function dfs(_interface) {
            const isArray = getType_1.default(_interface) === 'array';
            let _res = isArray ? [] : {};
            if (isArray) {
            ...
            for (let k in _interface) {
                // @ts-ignore
                curType = getType_1.default(_interface[k]);
                switch (curType) {
                    case 'object':
...
class MyCodeLens {
    provideCodeLenses(document, token) {
        ...
        const codeLensLine = document.lineCount - 1;
        for (let i = 0; i < codeLensLine; ++i) {
            const lineContent = document.lineAt(i).text;
            if (lineContent.indexOf('@ac-end') >= 0) {
                used = true;
                range = new vscode_1.Range(i, 0, i, 0);
                codeLensRes.push(new vscode_1.CodeLens(range, {
                    title: 'convert code',
...
exports.default = new MyCodeLens();
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const vscode = __webpack_require__(1);
const parse_1 = __webpack_require__(5);
exports.default = (context) => {
    const state = context.workspaceState;
    return vscode.commands.registerCommand('api-complete.generateCode', (uri, endLen) => {
        var _a, _b;
        if (!vscode.workspace.workspaceFolders) {
            return vscode.window.showInformationMessage(`Open a folder or workspace... (File -> Open Folder)`);
        }
        if (!vscode.workspace.workspaceFolders.length) {
            return vscode.window.showInformationMessage(`You've not added any folder in the workspace`);
        }
        if (!uri || !endLen) {
            return;
        }
        ...
        for (let i = endLen; i >= 0; --i) {
            let lineContent = document.lineAt(i).text;
            if (lineContent.indexOf('@ac-start') >= 0) {
                (_b = vscode.window.activeTextEditor) === null || _b === void 0 ? void 0 : _b.insertSnippet(new vscode.SnippetString(parse_1.default(document.getText(new vscode.Range(i, 0, endLen, lineContent.length)), state)), new vscode.Range(endLen + 1, 0, endLen + 1, 0));
                break;
            }
        }
    });
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const getCamelCase_1 = __webpack_require__(6);
const json2interface_1 = __webpack_require__(7);
const getCode_1 = __webpack_require__(9);
const getWorkspacePath_1 = __webpack_require__(10);
const commonMethod = [
    'get',
    'post',
    'delete',
...
exports.default = (text, state) => {
    const apiKey = getWorkspacePath_1.default();
    const textArr = text
        .replace(startFlag, '')
        .replace(comment, '')
        .split(/\n/g)
            ...
            while (stack.length) {
                ...
                if (curText.startsWith('@ac-')) {
                    ...
                    if (curText.includes('method-url')) {
                        ...
                        requestInfo.url = url;
                        requestInfo.name = `fetch${getCamelCase_1.default(url.split('/').slice(-2).join('/'))}`;
                    }
                    else {
                        // 有可能还没获取到name
                        requestInfo[curText] = partText;
                    }
                    break;
                }
                else {
    ...
    state.update(apiKey, Object.assign(Object.assign({}, state.get(apiKey, {})), { [requestInfo.url]: Object.assign({}, requestInfo) }));
    requestInfo.request = json2interface_1.default(JSON.parse(`${requestInfo.request}`), requestInfo.name);
    requestInfo.response = json2interface_1.default(JSON.parse(`${requestInfo.response}`), requestInfo.name, 'Res');
    return getCode_1.default(requestInfo);
};
...
function setSpace(count, base = 4) {
    return noBreakSpace.repeat(count * base);
}
function json2interface(json, name, suffix = 'Param') {
    function dfs(_json, level = 1) {
        const space = setSpace(level);
        const isObj = getType_1.default(_json) === 'object';
        let res = '';
        let type;
        for (let k in _json) {
            type = getType_1.default(_json[k]);
            switch (type) {
                case 'array':
                    if (getType_1.default(_json[k][0]) === 'object') {
                        res += `${space}${k}:${noBreakSpace}{
${dfs(_json[k][0], level + 1)}${space}}[];\n`;
                    }
                    else {
                        res += `${space}${k}:${noBreakSpace}${getType_1.default(_json[k][0])}[];\n`;
                    }
                    break;
                case 'object':
                    res += `${space}${k}:${noBreakSpace}{
...
exports.default = json2interface;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.default = (unknown) => Object.prototype.toString.call(unknown).split('').slice(8, -1).join('').toLowerCase();
...
const vscode = __webpack_require__(1);
exports.supportedLanguages = ['typescript', 'javascript'];
exports.default = (requestInfo) => {
    var _a;
    const fileType = (_a = vscode.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.document.languageId;
    return generates[fileType](requestInfo);
};
const generates = {
    typescript(requestInfo) {
        return `
${requestInfo.request}
...
exports.default = () => {
    var _a;
    const curFilePath = (_a = vscode.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.document.fileName;
    const key = vscode.workspace.workspaceFolders.filter((folder) => curFilePath === null || curFilePath === void 0 ? void 0 : curFilePath.includes(folder.uri.path))[0].uri.path;
    return key;
};
...
exports.default = () => {
    return vscode.commands.registerCommand('api-complete.addApi', (uri) => {
        ...
        (_b = vscode.window.activeTextEditor) === null || _b === void 0 ? void 0 : _b.insertSnippet(new vscode.SnippetString(`// @ac-start
// @ac-method-url
// post /demo/url
//
// @ac-request
...
const getWorkspacePath_1 = __webpack_require__(10);
const randomRes_1 = __webpack_require__(2);
exports.default = (context) => {
    const state = context.workspaceState;
    return vscode.commands.registerCommand('api-complete.mock', (uri) => {
        ...
        if (!((_a = vscode.workspace.workspaceFolders) === null || _a === void 0 ? void 0 : _a.length)) {
            return vscode.window.showInformationMessage(`Need folder in the workspace`);
        }
        const apiKey = getWorkspacePath_1.default();
        const app = new koa();
        app.use((ctx) => __awaiter(void 0, void 0, void 0, function* () {
            const mockApi = state.get(apiKey, {});
            ...
            if (mockApi[url] && mockApi[url].method === method.toLowerCase()) {
                ctx.body = randomRes_1.default(mockApi[url].response);
            }
            else {
                ctx.body = 'fail';
            }
...
const isGeneratorFunction = __webpack_require__(14);
const debug = __webpack_require__(15)('koa:application');
const onFinished = __webpack_require__(25);
const response = __webpack_require__(27);
const compose = __webpack_require__(49);
const context = __webpack_require__(50);
const request = __webpack_require__(74);
const statuses = __webpack_require__(40);
const Emitter = __webpack_require__(54);
const util = __webpack_require__(21);
const Stream = __webpack_require__(44);
const http = __webpack_require__(73);
const only = __webpack_require__(47);
const convert = __webpack_require__(87);
const deprecate = __webpack_require__(94)('koa');
const { HttpError } = __webpack_require__(51);
...
const tty = __webpack_require__(20);
...
var first = __webpack_require__(26)
...
const contentDisposition = __webpack_require__(28);
const getType = __webpack_require__(32);
const onFinish = __webpack_require__(25);
const escape = __webpack_require__(37);
const typeis = __webpack_require__(38).is;
const statuses = __webpack_require__(40);
const destroy = __webpack_require__(42);
const assert = __webpack_require__(45);
const extname = __webpack_require__(29).extname;
const vary = __webpack_require__(46);
const only = __webpack_require__(47);
const util = __webpack_require__(21);
const encodeUrl = __webpack_require__(48);
const Stream = __webpack_require__(44);
...
var basename = __webpack_require__(29).basename
...
const mimeTypes = __webpack_require__(33);
const LRU = __webpack_require__(36);
...
var typer = __webpack_require__(39)
...
var ReadStream = __webpack_require__(43).ReadStream
...
const util = __webpack_require__(21);
const createError = __webpack_require__(51);
const httpAssert = __webpack_require__(61);
const delegate = __webpack_require__(67);
const statuses = __webpack_require__(40);
const Cookies = __webpack_require__(68);
...
var relative = __webpack_require__(29).relative
...
var eql = __webpack_require__(64)
...
var deprecate = __webpack_require__(69)('cookies')
var Keygrip = __webpack_require__(70)
...
var compare = __webpack_require__(71)
...
const URL = __webpack_require__(75).URL;
const net = __webpack_require__(76);
const accepts = __webpack_require__(77);
const contentType = __webpack_require__(83);
const stringify = __webpack_require__(75).format;
const parse = __webpack_require__(84);
const qs = __webpack_require__(85);
const typeis = __webpack_require__(38);
const fresh = __webpack_require__(86);
const only = __webpack_require__(47);
const util = __webpack_require__(21);
...
const co = __webpack_require__(88)
const compose = __webpack_require__(89)
...
const Promise = __webpack_require__(90)
...
module.exports = __webpack_require__(91)().Promise
...
module.exports = __webpack_require__(92)(global, loadImplementation);
...
function loadImplementation(implementation){
  ...
  if(shouldPreferGlobalPromise(implementation)){
    ...
    var lib = __webpack_require__(93)(implementation)
    impl = {
...
const vscode = __webpack_require__(1);
const codeLens_1 = __webpack_require__(3);
const generateCode_1 = __webpack_require__(4);
const addApi_1 = __webpack_require__(11);
const mock_1 = __webpack_require__(12);
const getWorkspacePath_1 = __webpack_require__(10);
function activate(context) {
    ...
    vscode.languages.registerCodeLensProvider({ scheme: 'file' }, codeLens_1.default);
    // 便于vscode被关闭的时进行回收
    context.subscriptions.push(generateCode_1.default(context));
    context.subscriptions.push(addApi_1.default());
    context.subscriptions.push(mock_1.default(context));
    context.subscriptions.push(vscode.commands.registerCommand('api-complete.clearMockData', () => {
        state.update(apiKey, {});
    }));
}
